...
class BoatosOrgTestCase(unittest.TestCase):
    ...
    def test_page_with_red_text(self):
        link = "http://www.boatos.org/saude/enfermeira-vacina-febre-amarela-farsa.html"
        result = scrape_mocked("red_text.html", link, scrape_hoax)
        ...
        self.assertEqual(str(result['timestamp']), '2018-01-20 17:29:27')
        self.assertEqual(trim(result['hoax']), trim(expected))

    def test_page_with_italic_text(self):
        link = "http://www.boatos.org/politica/crivella-fechar-sao-cristovao.html"
        result = scrape_mocked("italic_text.html", link, scrape_hoax)
        ...
        self.assertEqual(result['link'], link)
        self.assertEqual(str(result['timestamp']), '2018-01-28 16:33:11')
        self.assertEqual(trim(result['hoax']), trim(expected))

    def test_page_with_multiple_texts(self):
        link = "http://www.boatos.org/politica/teori-zavascki-foi-assassinado.html"
        result = scrape_mocked("multiple_texts.html", link, scrape_hoax)
        ...
        self.assertEqual(result['link'], link)
        self.assertEqual(str(result['timestamp']), '2017-01-20 04:21:18')
        self.assertEqual(trim(result['hoax']), trim(expected))
    ...
    def test_returns_empty_for_no_text(self):
        ...
        self.assertEqual(result['hoax'], "")

    def test_returns_empty_when_it_should_ignore_scrapped_paragraph(self):
        result = scrape_mocked(
            "ignored_text.html",
            "http://www.boatos.org/ciencia/foto-buraco-meteorito-pirassununga.html",
            scrape_hoax)

        self.assertEqual(result['hoax'], "")

    def test_find_links_from_search_page(self):
        result = scrape_mocked("search_page.html",
                               "http://www.boatos.org/page/1?s=%23boato",
                               find_links_from_search_page)

        self.assertEqual(result, [
            'http://www.boatos.org/saude/matar-macacos-febre-amarela-humanos.html',
            'http://www.boatos.org/saude/vacina-febre-amarela-figado-sorocaba.html',
            'http://www.boatos.org/politica/crivella-fechar-sao-cristovao.html',
            'http://www.boatos.org/mundo/secretario-geral-onu-ameaca-brasil.html',
            'http://www.boatos.org/politica/mandado-prisao-lula-moro.html',
            'http://www.boatos.org/politica/marise-valeria-lula-carta.html',
            'http://www.boatos.org/entretenimento/cantor-leonardo-comemora-lula.html',
            'http://www.boatos.org/entretenimento/marieta-severo-texto-lula.html',
            'http://www.boatos.org/brasil/motoristas-uber-bala-sonifero.html',
            'http://www.boatos.org/brasil/bruxo-sacrificava-criancas-globo.html',
            'http://www.boatos.org/tecnologia/maior-rede-cosmeticos-lapis-delineador.html',
            'http://www.boatos.org/saude/medica-butantan-vacina-febre.html',
            'http://www.boatos.org/politica/sergio-moro-carta-povo-brasileiro.html',
            'http://www.boatos.org/religiao/video-atentado-igreja-nigeria.html',
            'http://www.boatos.org/religiao/ataque-igreja-bolivia-video.html',
            'http://www.boatos.org/tecnologia/spotify-dando-contas-premium.html',
            'http://www.boatos.org/politica/alvaro-dias-caixa-fraude.html',
            'http://www.boatos.org/politica/policia-federal-baus-porto-santos.html',
